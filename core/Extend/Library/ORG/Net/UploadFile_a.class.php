...
class UploadFile {//类定义开始
    ...
    public $thumb   =  false;
    // 图库类包路径
    public $imageClassPath = 'ORG.Util.Imagec';
    // 缩略图最大宽度
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    Imagec::thumb($filename,$thumbname,'',$thumbWidth[$i],$thumbHeight[$i],true);
   ...
   //指定文件生成缩略图
    public function thumb_cut($img,$width=300,$height=300) {
        //$filename = $file['savepath'].$file['savename'];
         $filename=$img;
		 $file['extension']=$this->getExt($filename);
		  $path_arr=explode('/',$img);
		  $file['savename']=array_splice($path_arr,-2,2);
		  $file['name']=array_pop($path_arr);
		  $file['savename']= $file['savename'][0].'/'.$file['savename'][1];
        ...
        if(in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                //是图像文件生成缩略图
                $thumbWidth		=	explode(',',$width);
                $thumbHeight		=	explode(',',$height);
                $thumbPrefix		=	explode(',',$this->thumbPrefix);
                $thumbSuffix = explode(',',$this->thumbSuffix);
                $thumbFile			=	explode(',',$file['name']);			
                 $thumbPath    =file_path(C('ATTACHMENT'));
                // 生成图像缩略图

                import($this->imageClassPath);
                $realFilename  =  $this->autoSub?basename($file['savename']):$file['savename'];
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {               	
                    $thumbname	=	$thumbPath.$thumbPrefix[$i].substr($realFilename,strrpos($realFilename, '/')+1,strrpos($realFilename, '.'));
                    $image_c=new Image_c();
                    $image_c->thumb($filename,$thumbname,'',$thumbWidth[$i],$thumbHeight[$i],true);
                }
                if($this->thumbRemoveOrigin) {
                    // 生成缩略图之后删除原图
                    unlink($filename);
