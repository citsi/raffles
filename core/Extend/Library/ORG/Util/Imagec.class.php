...
class Imagec {
     ...
     */
     static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        // 获取原图信息
        $info = Imagec::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
            ...
            unset($info);
            //--------------------------------------最小边缩略
            $min_side=min($srcWidth,$srcHeight);
            $min_side_thumb=min($maxWidth,$maxHeight);
            $scale = $min_side_thumb/$min_side; // 计算缩放比例
            if ($scale >= 1 || $min_side<=$min_side_thumb) {
                //--------------------------------------
                // 超过原图大小不再缩略
                $width = $srcWidth;
                $height = $srcHeight;
            ...
            //	$ch= ($maxHeight-$height)/2;
            //}
            // 复制图片
            if (function_exists("ImageCopyResampled"))
                imagecopyresampled($thumbImg, $srcImg, $cw, $ch, 0, 0, $width, $height, $srcWidth, $srcHeight);
            else
                imagecopyresized($thumbImg, $srcImg, $cw, $ch, 0, 0, $width, $height, $srcWidth, $srcHeight);
            //-----------------------------------------------------------------------
            // 对jpeg图形设置隔行扫描
    ...
    static function buildString($string, $rgb=array(), $filename='', $type='png', $disturb=1, $border=true) {
        ...
        Imagec::output($im, $type, $filename);
    }
